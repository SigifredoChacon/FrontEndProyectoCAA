...
import { getApplication, updateApplication } from '../services/applicationService.jsx';
import { updateAsset } from "../services/assetService.jsx";
import AcceptApplicationModal from "../components/Application/AcceptApplicationModal.jsx";
...
function AllPendingApplicationPage() {
    ...
    const [requests, setRequests] = useState([]); // Lista de solicitudes pendientes
    const [isModalOpen, setIsModalOpen] = useState(false); // Estado de apertura del modal
    const [selectedArchivoSolicitud, setSelectedArchivoSolicitud] = useState(null); // Archivo de solicitud seleccionado para el modal
    const [selectedApplication, setSelectedApplication] = useState(null); // Solicitud seleccionada para aceptar/rechazar

    // Efecto para cargar solicitudes cuando se actualiza selectedApplication
    useEffect(() => {
        fetchRequests();
    }, [selectedApplication]);

    // Función para obtener solicitudes pendientes y detalles de activos y usuarios
    // Entrada: ninguna
    // Salida: actualiza requests con solicitudes pendientes y detalles adicionales
    ...
    // Función para aceptar una solicitud y actualizar su estado
    // Entrada: ninguna
    // Salida: confirma la aceptación de la solicitud y actualiza la lista de solicitudes
    const handleAcceptApplication = async () => {
        await Swal.fire({
            title: '¡Aceptar solicitud!',
            text: '¿Estás seguro de que deseas aceptar esta solicitud? Una vez aceptada, no podrás revertir esta acción.',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                try {
                    await updateApplication(selectedApplication.idSolicitud, { estado: "Aceptada" });
                    await fetchRequests();
    ...
    // Función para rechazar una solicitud, actualizar su estado y enviar justificación
    // Entrada: justification (justificación proporcionada al rechazar)
    // Salida: rechaza la solicitud, actualiza la lista y notifica al usuario
    const handleRejectApplication = async (justification) => {
        await Swal.fire({
            title: '¡Rechazar solicitud!',
            text: '¿Estás seguro de que deseas rechazar esta solicitud? Una vez rechazada, no podrás revertir esta acción.',
            icon: 'warning',
            showConfirmButton: true,
            confirmButtonText: 'Aceptar',
        }).then(async (result) => {
            if (result.isConfirmed) {
                try {
                    await updateApplication(selectedApplication.idSolicitud, { estado: "Rechazada" });
                    ...
                    await Swal.fire({
                        ...
                        text: 'Se ha rechazado la solicitud con éxito. Se le ha informado al usuario vía correo electrónico',
                        icon: 'success',
                        timer: 2500,
                        timerProgressBar: true,
                        showConfirmButton: false,
    ...
    // Función para abrir el modal de aceptación/rechazo de solicitud
    // Entrada: archivoSolicitud (ruta del archivo PDF de la solicitud), application (solicitud seleccionada)
    // Salida: abre el modal y actualiza el archivo y la solicitud seleccionados
    ...
    // Cierra el modal y restablece archivo y solicitud seleccionados
    // Entrada: ninguna
    // Salida: cierra el modal y restablece los valores seleccionados
