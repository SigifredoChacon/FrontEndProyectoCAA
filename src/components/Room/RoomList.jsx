...
function RoomList({ onEdit }) {
    ...
    const fetchRooms = async () => {
        try {
            const data = await getRooms(); // Llama al servicio para obtener la lista de usuarios
            setRooms(data); // Actualiza el estado con los datos obtenidos
        } catch (error) {
                ...
                <TableBody>
                    {rooms.map((room) => (
                        <TableRow key={room.idSala}>
                            ...
                            <TableCell>{room.Imagen}</TableCell>
